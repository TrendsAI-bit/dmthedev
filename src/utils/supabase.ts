...
export interface EncryptedMessage {
  ...
  createdAt?: string;
  usesSignatureDerivedKey?: boolean;
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        uses_signature_derived_key: message.usesSignatureDerivedKey || false,
        encrypted_for_public_key: message.encryptedForPublicKey
      });
...
export async function fetchMessagesForDeployer(recipientAddress: string): Promise<EncryptedMessage[]> {
  try {
    ...
    return (data || []).map(msg => ({
      ...
      usesSignatureDerivedKey: msg.uses_signature_derived_key || false,
      encryptedForPublicKey: msg.encrypted_for_public_key
    }));
  } catch (error) {
